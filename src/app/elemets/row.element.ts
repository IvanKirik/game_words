...
export class RowElement implements IBasicElement {
    ...
    constructor(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, words: WordRound) {
        this.ctx = ctx;
        this.x = x;
        this.y = y;
        this.size = size;
    ...
    private create(): ColElement[] {
        ...
        this.word.word.split('').forEach((letter, index) => {
            ...
            cools.push(new ColElement(this.ctx, col, this.y, this.size, letter, !this.word.completed));
        })
        return cools;
    }
